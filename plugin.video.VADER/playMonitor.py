...
class Service(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        ...
        if salts_resume:
            log_utils.log("Salts Local Resume: Resume Time: %s Total Time: %s" % (salts_resume, self._totalTime),
                          log_utils.LOGDEBUG, COMPONENT)
            self.seekTime(float(salts_resume))
        elif trakt_resume:
            resume_time = float(trakt_resume) * self._totalTime / 100
            log_utils.log("Salts Trakt Resume: Percent: %s, Resume Time: %s Total Time: %s" % (
            trakt_resume, resume_time, self._totalTime), log_utils.LOGDEBUG, COMPONENT)
...
def show_next_up():
    ...
    if token and xbmc.getInfoLabel('Container.PluginName') == kodi.get_id() and xbmc.getInfoLabel(
        ...
        if sf_begin and (time.time() - sf_begin) >= int(kodi.get_setting('next_up_delay')):
            ...
            if 'trakt_id' in queries:
                ...
                try:
                    ...
                    trakt_timeout = 20
                trakt_api = Trakt_API(token, kodi.get_setting('use_https') == 'true', list_size, trakt_timeout,
                ...
                if 'next_episode' in progress and progress['next_episode']:
                    if progress['completed'] or kodi.get_setting('next_unwatched') == 'true':
                        ...
                        if kodi.get_setting('next_time') != '0':
                            date_time = '%s@%s' % (
                            date, utils2.make_time(utils.iso_2_utc(next_episode['first_aired']), 'next_time'))
                        else:
                            date_time = date
                        msg = '[[COLOR deeppink]%s[/COLOR]] - %sx%s' % (
                        date_time, next_episode['season'], next_episode['number'])
...
while not xbmc.abortRequested:
    try:
        ...
        if errors >= MAX_ERRORS:
            log_utils.log('Service: Error (%s) received..(%s/%s)...Ending Service...' % (e, errors, MAX_ERRORS),
                          log_utils.LOGERROR, COMPONENT)
            break
        else:
            log_utils.log('Service: Error (%s) received..(%s/%s)...Continuing Service...' % (e, errors, MAX_ERRORS),
                          log_utils.LOGERROR, COMPONENT)
    else:
        errors = 0

...
def add_item(action="",title="",plot="",url="",thumbnail="",fanart="",show="",episode="",extra="",page="",info_labels=None,isPlayable=False,folder=True):
    ...
    if url.startswith("plugin://"):
        itemurl=url; listitem.setProperty('IsPlayable','true');
        ...
        xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=itemurl,listitem=listitem,isFolder=folder)
    else: itemurl='%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s' % (sys.argv[0],action,urllib.quote_plus(title),urllib.quote_plus(url),urllib.quote_plus(thumbnail),urllib.quote_plus(plot),urllib.quote_plus(extra),urllib.quote_plus(page));
    xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=itemurl,listitem=listitem,isFolder=folder)


def add_playable(title="",plot="",url="",thumbnail="",fanart="",show="",episode="",extra="",page="",info_labels=None,isPlayable=False,folder=True):
...
def add_playable(title="",plot="",url="",thumbnail="",fanart="",show="",episode="",extra="",page="",info_labels=None,isPlayable=False,folder=True):
    ...
    if fanart!="":
        ...
        listitem.setProperty('IsPlayable', 'true');
        itemurl='title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s' % (urllib.quote_plus(title),urllib.quote_plus(url),urllib.quote_plus(thumbnail),urllib.quote_plus(plot),urllib.quote_plus(extra),urllib.quote_plus(page));
        xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=url,listitem=listitem,isFolder=folder)
...
def set_setting(name,value):
    log("set_setting name='"+name+"','"+value+"'")
...
def set_view(view_mode, view_code=0):
    ...
    try:
        ...
        log("Unable to find view code for view mode "+str(view_mode)+" and skin "+skin_name)

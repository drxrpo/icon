...
addon_path = xbmc.translatePath(('special://home/addons/plugin.video.it')).decode('utf-8')
module_path = xbmc.translatePath(('special://home/addons/script.module.it')).decode('utf-8')


def main():
    fum_ver = xbmcaddon.Addon(id='script.module.it').getAddonInfo('version')
    updated = xbmcaddon.Addon(id='plugin.video.it').getSetting('module_base')
    ...
    log('IT Scraper Path: %s' % (str(scraper_path)), LOGINFO)
    try:
        xml = openfile(settings_xml_path)
    except Exception:
        failure = traceback.format_exc()
        log('IT Service - Exception: \n %s' % (str(failure)), LOGINFO)
        return
    ...
    xbmcaddon.Addon(id='plugin.video.it').setSetting('module_base', fum_ver)
    ...
    log('13 Scraper Path: %s' % (str(scraper_path)), LOGINFO)
    try:
        xml = openfile(settings_xml_path)
    except Exception:
        failure = traceback.format_exc()
...
DEBUGPREFIX = '[COLOR red][ IT DEBUG ][/COLOR]'
...
def syncTraktLibrary():
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.it/?action=tvshowsToLibrarySilent&url=traktcollection')
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.it/?action=moviesToLibrarySilent&url=traktcollection')

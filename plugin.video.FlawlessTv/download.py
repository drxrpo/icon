...
def doDownload(url, dest, title, referer='', agent='', cookie='', quiet=False):
    ...
    iptv_utils.log('Download originally resumable: %r' % resumable)
    resumable = True

    iptv_utils.log('Download Header')
    iptv_utils.log(resp.headers)
    if resumable:
        iptv_utils.log('Download is resumable')
    ...
    while True:
        ...
        if percent >= notify:
            ...
            iptv_utils.log('Download percent : %s %s %dMB downloaded : %sMB File Size : %sMB' % (str(percent)+'%', dest, mb, downloaded / 1000000, content / 1000000))
        ...
        try:        
            ...
            if not chunk:
                if percent < 99:
                    ...
                    f.close()
                    iptv_utils.log('%s download complete' % (dest))
                    if not quiet:
        ...
        if error:
            errors += 1
            count  += 1
            iptv_utils.log('%d Error(s) whilst downloading %s' % (count, dest))
            xbmc.sleep(sleep*1000)

        if (resumable and errors > 0) or errors >= 10:
            if (not resumable and resume >= 50) or resume >= 500:
                #Give up!
                iptv_utils.log('%s download canceled - too many error whilst downloading' % (dest))
                if not quiet:
            ...
            if resumable:
                chunks  = []
                #create new response
                iptv_utils.log('Download resumed (%d) %s' % (resume, dest))

...
def my_furk_audio_favourites():
    ...
    for item in results:
        try:
            ...
            cm.append(("[B]Remove from Fen Favourites[/B]",'XBMC.RunPlugin(%s)' % build_url(con_remove_favs)))
    ...
    for count, item in enumerate(files, 1):
        try:
            ...
            if not uncached:
                ...
                if params.get('db_type') == 'audio': cm.append(("[B]Add to Fen Favourites[/B]",'XBMC.RunPlugin(%s)' % build_url(con_add_favs)))
...
def t_file_browser(item_id, filtering_list=None, source=None):
    ...
    if settings.auto_play() or settings.auto_resolve():

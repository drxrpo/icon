...
class FurkAPI:
    ...
    def search(self, query):
        try:
            ...
            link = (self.base_link + self.search_link \
                % (api_key, query, 'extended', self.mod_level, search_in))
            cache = _cache.get("fen_%s_%s" % ('FURK_SEARCH', query))
            if cache:
                files = cache
            else:
                p = self._get(link)
                if p['status'] != 'ok':
                    return
                files = p['files']
                _cache.set("fen_%s_%s" % ('FURK_SEARCH', query), files,
    ...
    def direct_search(self, query):
        try:
            ...
            cache = _cache.get("fen_%s_%s" % ('FURK_SEARCH_DIRECT', query))
            if cache:
                files = cache
            else:
                p = self._get(link)
                if p['status'] != 'ok':
                    return
                files = p['files']
                _cache.set("fen_%s_%s" % ('FURK_SEARCH_DIRECT', query), files,
                    expiration=datetime.timedelta(hours=2))
            return files
        except: return

...
def build_kodi_library_recently_added(db_type):
    ...
    if db_type in ('movie', 'movies'):
        ...
        from resources.lib.indexers.tvshows import build_episode, make_fresh_tvshow_meta
        from resources.lib.modules.workers import Thread
        def process_eps(item):
            results.append(build_episode({"season": int(item['season']), "episode": int(item['episode']), 'order': item['order'], "meta": make_fresh_tvshow_meta('tvdb_id', item['media_id'])}))
        db_type = 'episodes'
        ...
        try:
            ...
            threads = []
            for item in recent_added: threads.append(Thread(process_eps, item))

...
def mainlist(item):
    logger.info("[thegroove360.ilgiramondo] mainlist")
    itemlist = [Item(channel=__channel__, title="[COLOR azure]Video di Viaggi[/COLOR]", action="peliculas",
                     url=host + "/video-vacanze-viaggi/",
                     thumbnail="https://raw.githubusercontent.com/stesev1/channels/master/images/channels_icon/airplane.png")]

    return itemlist


def peliculas(item):
    logger.info("[thegroove360.ilgiramondo] peliculas")
    itemlist = []

...
addon = xbmcaddon.Addon('plugin.video.OblivionStreams')
...
SOURCES = [{"url": "http://pastebin.com/raw/ed9ZWyWK", "fanart": "http://oblivionbuildz.com/fanart.jpg", "genre": "Entertainment", "date": "14.11.2015", "credits": "Cypher for for some coding and all the other devs for helping when needed", "thumbnail": "http://oblivionbuildz.com/Oblivionlogo.png"}]
...
def addon_log(string):
	if debug == 'true':
		xbmc.log("[plugin.video.OblivionStreams-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
		try:
			...
			if hasattr(e, 'code'):
				...
				xbmc.executebuiltin("XBMC.Notification(OblivionStreams,We failed with error code - "+str(e.code)+",10000,"+icon+")")
			elif hasattr(e, 'reason'):
				addon_log('We failed to reach a server.')
				addon_log('Reason: %s' %e.reason)
				xbmc.executebuiltin("XBMC.Notification(OblivionTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
		if os.path.exists(favorites) == True:
			...
			if FAV == "[]":
				...
				addDir('[COLOR green][B]Favourites[/COLOR][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
				addDir('','',100,'',FANART,'','','','')
...
def addSource(url=None):
		...
		xbmc.executebuiltin("XBMC.Notification(OblivionTV,New source added.,5000,"+icon+")")
		if not url is None:
...
def Decrypt_Link(other,channel_name,stream_url,total):
	 DeCrypt_Multi = (stream_url).replace('{IX},','http').replace('{UD},','.com')
	 ...
	 Decrypt8 = (Decrypt7).replace('{UJ},','-').replace('{XD},','_').replace('HAVE_A_NICE_DAY_NOW','TV')
	 if 'type' in other:
		  stream_url = Decrypt8
	 ...
	 stream_url = Decrypt8
	 addLink(stream_url, channel_name,thumbnail,'','','','','',None,'',total) # dd678faae9ac167bc83abf78e5cb2f3f0688d3a3
...
def urlsolver(url):
	...
	try:
		...
		xbmc.executebuiltin("XBMC.Notification(OblivionTV,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

	resolved=genesisresolvers.get(url).result
	if url == resolved or resolved is None:
		#import
		xbmc.executebuiltin("XBMC.Notification(OblivionTV,Using Urlresolver module.. - ,5000)")
		import urlresolver
...
def download_file(name, url):
		if addon.getSetting('save_location') == "":
			xbmc.executebuiltin("XBMC.Notification('OblivionTV','Choose a location to save files.',15000,"+icon+")")
			addon.openSettings()
		params = {'url': url, 'download_path': addon.getSetting('save_location')}
		downloader.download(name, params)
		dialog = xbmcgui.Dialog()
		ret = dialog.yesno('OblivionTV', 'Do you want to add this file as a source?')
		if ret:
...
def search(site_name,search_term=None):
	...
	if 'youtube' in site_name:
		...
		if res == 'True':
			...
			xbmc.executebuiltin("XBMC.Notification(OblivionTV,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
		...
		if showcontext:
			...
			if showcontext == 'fav':
				contextMenu.append(
					('Remove from channel OblivionTV','XBMC.RunPlugin(%s?mode=6&name=%s)'
				...
				contextMenu.append(('Add to OblivionTV Favorites','XBMC.RunPlugin(%s)' %fav_params))
			liz.addContextMenuItems(contextMenu)
...
def shadow(url):
	...
	block = re.compile('<div class="SideCategoryListClassic">(.+?)<div class="afterSideCategoryList"></div>',re.DOTALL).findall(html)
	match = re.compile('href="(.+?)">(.+?)</a>',re.DOTALL).findall(str(block))
	for url, name in match:
		addDir(name,url,100037,icon,fanart,'','','','','')

def shadow2(url):
	html = requests.get(url).content
	#print html
	match = re.compile('<div class="ProductDetails".+?href="(.+?)".+?>(.+?)<',re.DOTALL).findall(html)
	Page = re.compile('id="CategoryPagingTop">.+?<div class="FloatRight">.+?href="(.+?)">Next',re.DOTALL).findall(html)
	for u , n in match:
		if not'<' in n :
			n = n
			PLAY(n,u,100038,icon,fanart,'','')
	for nxt in Page:
		addDir('Next Page',nxt,100037,icon,fanart,'','','','','')

def shadow3(url,name):
	html = requests.get(url).content
	#print html
	match = re.compile("<iframe src='(.+?)'").findall(html)
	for link in match:
		#print link
		gt_m3u(url,name,link)

def gt_m3u(u,n,link):
	#print u
	f4m =  'plugin://plugin.video.f4mTester/?streamtype=TSDOWNLOADER&amp;url='
	headers = {"host":"shadow.go.ro",
                 "referer":u,
	...
	for play in match:
		play = play.replace('.m3u8','.ts')
		Big_Resolve(n.replace('&amp;','&'),f4m+play+';name=Oblivion TV') # n.replace('&amp;','&')+'\n'+f4m+play+';name=Oblivion TV'

def AutoIptv():
    HTML2 = requests.get('http://autoiptv.net/playlists/').content
    match = re.compile('<h1 class="title">(.+?)</h1>.+?<a class="playlist" href="([^"]*)".+?<a class="playlist" href="([^"]*)".+?<br><br>',re.DOTALL).findall(HTML2)
    for name,url,url2 in match:
    	if 'Adult' in name or 'Series' in name or '24/7' in name:
        	addDir(name,url2,1,icon,fanart,'','','','','')
        else:
        	addDir(name,url,100041,icon,fanart,'','','','','')

def AutoIptv2(name,url):
    #print url
    try:
    	f4m =  'plugin://plugin.video.f4mTester/?streamtype=TSDOWNLOADER&amp;url='
        country = name
        #addDir(country,url,100041,icon,fanart,'','','','','')
        HTML = requests.get(url).content
        match = re.compile('#EXTINF:.+?tvg-name="([^"]*)" tvg-logo="([^"]*)" group-title="([^"]*)",(.+?)\n(.+?)\n').findall(HTML)
        match2 = re.compile('#EXTINF:.+?tvg-logo="([^"]*)" group-title="([^"]*)",(.+?)\n(.+?)\n').findall(HTML)
        match3 = re.compile('#EXTINF:.+?, group-title="([^"]*)" tvg-logo="",(.+?)\n(.+?)\n').findall(HTML)
        match4 = re.compile('#EXTINF:.+?, group-title="([^"]*)" tvg-logo="([^"]*)",(.+?)\n(.+?)\n').findall(HTML)
        match5 = re.compile('<title>(.+?)</title>.+?<link>(.+?)</link>',re.DOTALL).findall(HTML)
        match6 = re.compile('#EXTINF:.+? group-title="([^"]*)",(.+?)\n(.+?)\n').findall(HTML)
        for name,img,group,name2,url in match:
            if country in group:
                url = url.replace('.m3u8','.ts')
                #Big_Resolve(n.replace('&amp;','&'),f4m+play+';name=Oblivion TV')
            	PLAY(name+name2,f4m+url+';name=Oblivion TV',906,img,fanart,'','')
                
        for img,group,name2,url in match2:
            if country in group:
            	url = url.replace('.m3u8','.ts')
            	PLAY(name2,f4m+url+';name=Oblivion TV',906,img,fanart,'','')
                #print img,group,name2,url
        for group,name2,url in match3:
            if country in group:
            	url = url.replace('.m3u8','.ts')
            	PLAY(name2,f4m+url+';name=Oblivion TV',906,icon,fanart,'','')
                #print group,name2,url
        for group,img,name2,url in match4:
            
            if country in group:
            	url = url.replace('.m3u8','.ts')
            	PLAY(name2,f4m+url+';name=Oblivion TV',906,img,fanart,'','')
                #print group,img,name2,url
        for name2,url in match5:
            #print name2,url
            url = url.replace('.m3u8','.ts')
            PLAY(name2,f4m+url+';name=Oblivion TV',906,icon,fanart,'','')
        for group,name2,url in match6:
            #print group
            if country in group:
            	url = url.replace('.m3u8','.ts')
            	PLAY(name2,f4m+url+';name=Oblivion TV',906,icon,fanart,'','')
...
def AutoIptv3(url):
    url = url
    TEXTCOL = 'deepskyblue'
    choices = ['[COLOR'+TEXTCOL+'](_) _No Group[/COLOR]','[COLOR'+TEXTCOL+'](UK) United Kingdom[/COLOR]','[COLOR'+TEXTCOL+'](US) United States[/COLOR]','[COLOR'+TEXTCOL+'](AL) Albania[/COLOR]','[COLOR'+TEXTCOL+'](AS) American Samoa[/COLOR]','[COLOR'+TEXTCOL+'](AR) Argentina[/COLOR]','[COLOR'+TEXTCOL+'](AU) Australia[/COLOR]','[COLOR'+TEXTCOL+'](AZ) Azerbaijan[/COLOR]','[COLOR'+TEXTCOL+'](BY) Belarus[/COLOR]','[COLOR'+TEXTCOL+'](BE) Belgium[/COLOR]','[COLOR'+TEXTCOL+'](BO) Bolivia[/COLOR]','[COLOR'+TEXTCOL+'](BR) Brazil[/COLOR]','[COLOR'+TEXTCOL+'](CM) Cameroon[/COLOR]','[COLOR'+TEXTCOL+'](CA) Canada[/COLOR]','[COLOR'+TEXTCOL+'](CO) Colombia[/COLOR]','[COLOR'+TEXTCOL+'](CZ) Czech Republic[/COLOR]','[COLOR'+TEXTCOL+'](DO) Dominican Republic[/COLOR]','[COLOR'+TEXTCOL+'](EC) Ecuador[/COLOR]','[COLOR'+TEXTCOL+'](FO) Faroe Islands[/COLOR]','[COLOR'+TEXTCOL+'](FR) France[/COLOR]','[COLOR'+TEXTCOL+'](DE) Germany[/COLOR]','[COLOR'+TEXTCOL+'](GR) Greece[/COLOR]','[COLOR'+TEXTCOL+'](IS) Iceland[/COLOR]','[COLOR'+TEXTCOL+'](IN) India[/COLOR]','[COLOR'+TEXTCOL+'](ID) Indonesia[/COLOR]','[COLOR'+TEXTCOL+'](IR) Iran[/COLOR]','[COLOR'+TEXTCOL+'](IT) Italy[/COLOR]','[COLOR'+TEXTCOL+'](LA) Laos[/COLOR]','[COLOR'+TEXTCOL+'](MO) Macau[/COLOR]','[COLOR'+TEXTCOL+'](MX) Mexico[/COLOR]','[COLOR'+TEXTCOL+'](NL) Netherlands[/COLOR]','[COLOR'+TEXTCOL+'](NE) Niger[/COLOR]','[COLOR'+TEXTCOL+'](PK) Pakistan[/COLOR]','[COLOR'+TEXTCOL+'](PA) Panama[/COLOR]','[COLOR'+TEXTCOL+'](PE) Peru[/COLOR]','[COLOR'+TEXTCOL+'](PL) Poland[/COLOR]','[COLOR'+TEXTCOL+'](PT) Portugal[/COLOR]','[COLOR'+TEXTCOL+'](RO) Romania[/COLOR]','[COLOR'+TEXTCOL+'](RU) Russia[/COLOR]','[COLOR'+TEXTCOL+'](SL) Sierra Leone[/COLOR]','[COLOR'+TEXTCOL+'](EX-YU) Slovenia[/COLOR]','[COLOR'+TEXTCOL+'](SO) Somalia[/COLOR]','[COLOR'+TEXTCOL+'](SP) Spain[/COLOR]','[COLOR'+TEXTCOL+'](SE) Sweden[/COLOR]','[COLOR'+TEXTCOL+'](CH) Switzerland[/COLOR]','[COLOR'+TEXTCOL+'](TH) Thailand[/COLOR]','[COLOR'+TEXTCOL+'](TR) Turkey[/COLOR]','[COLOR'+TEXTCOL+'](UA) Ukraine[/COLOR]','[COLOR'+TEXTCOL+'](VE) Venezuela[/COLOR]','[COLOR'+TEXTCOL+'](WW) World Wide[/COLOR]']
    choice = xbmcgui.Dialog().select('[COLOR'+TEXTCOL+']Select Country[/COLOR]', choices)
    if choice==0:
        name = '(_)'
    if choice==1:
        name = '(UK)'
...
def livescrape():
    addDir('[COLORred][B]SOME LINKS MAY NOT WORK AS THESE ARE NOT MANUALY ADDED[/COLOR][/B]','','',icon,fanart,'','','','','')
    ...
    mach = re.compile('<td width="30%" align=.+?22px;"><strong>(.+?)</strong></span></td>.+?<td width="48%">LINK</td>(.+?)<table width="100%" border="0">',re.DOTALL).findall(html)
    for chann, rest in mach:
        ...
        for watch, Plink in Links:
			...
			PLAY('[COLORdeepskyblue]'+chann+'[/COLOR]'+'\n'+'[COLORwhite]'+watch+'[/COLOR]',plinking,906,icon,fanart,'','','','')
...
if mode==None:
	...
	if url:
		...
		xbmc.executebuiltin("XBMC.Notification(OblivionTV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
	addon_log("youtubedl")
	try:
		import youtubedl
	except Exception:
		xbmc.executebuiltin("XBMC.Notification(OblivionTV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")

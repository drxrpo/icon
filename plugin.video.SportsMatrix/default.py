...
if _Edit.UseTMDB :
    if _Edit.TMDB_api != '':
        ...
        TMDB_api = addon.getSetting('tmdb_key')
    if  len(TMDB_api) == 0:
        tmdbapi = koding.YesNo_Dialog(title='TMDB API Key',message='API key is missing do you wish to enter in settings?')
        if tmdbapi:
...
if _Edit.DailyMotionIcon != '':
    DailyMotionIcon = _Edit.DailyMotionIcon
else:
    DailyMotionIcon = os.path.join(ART,'dailymotion.png')
if _Edit.DailyMotionFanart != '':
    DailyMotionFanart = _Edit.DailyMotionFanart
else:
    DailyMotionFanart = FANART
if _Edit.FavoriteIcon != '':
    FavoriteIcon = _Edit.FavoriteIcon
else:
    FavoriteIcon = os.path.join(ART,'favorite.png')
if _Edit.FavoriteFanart != '':
    FavoriteFanart = _Edit.FavoriteFanart
else:
    FavoriteFanart = FANART
if _Edit.HistoryIcon != '':
    HistoryIcon = _Edit.HistoryIcon
else:
    HistoryIcon = os.path.join(ART,'history.png')
if _Edit.HistoryFanart != '':
    HistoryFanart = _Edit.HistoryFanart
else:
    HistoryFanart = FANART
if _Edit.SettingIcon != '':
    SettingIcon = _Edit.SettingIcon
else:
    SettingIcon = os.path.join(ART,'tools.png')
if _Edit.SettingFanart != '':
    SettingFanart = _Edit.SettingFanart
else:
    SettingFanart = FANART
if _Edit.YouTubeIcon != '':
    YouTubeIcon = _Edit.YouTubeIcon
else:
    YouTubeIcon = os.path.join(ART,'youtube.png')
if _Edit.YouTubeFanart != '':
    YouTubeFanart = _Edit.YouTubeFanart
else:
    YouTubeFanart = FANART

if addon.getSetting("resolverURL") == "resolveurl":
    import resolveurl as RESOLVE
elif addon.getSetting("resolverURL") == "urlresolver":
    URLRES = xbmc.getCondVisibility('System.HasAddon(script.module.urlresolver)')
    if URLRES:
        import urlresolver as RESOLVE
    else:
        koding.dolog('urlresolver is not installed',line_info=True)
        xbmc.executebuiltin('urlresolver not installed swapping to ResolveUrl, Settings will also be changed,10000,%s'%icon)
        addon.setSetting('resolverURL', 'resolveurl')
        import resolveurl as RESOLVE 
koding.dolog('RESOLVE been used = %s'%addon.getSetting('resolverURL'),line_info=True)
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification("+str(addon_name)+",We failed with error code - "+str(e.code)+",10000,"+str(icon)+")")
            elif hasattr(e, 'reason'):
                koding.dolog('We failed to reach a server.',line_info=True)
                koding.dolog('Reason: '+str(e.reason),line_info=True)
                xbmc.executebuiltin("XBMC.Notification("+str(addon_name)+",We failed to reach a server. - "+str(e.reason)+",10000,"+str(icon)+")")
...
def SKindex():
    ...
    getSources()
    BYB.addDir('Tools and Settings','',36,SettingIcon ,SettingFanart,'Tools and Settings','','','')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
		
	
def getSources():
        if os.path.exists(favorites) == True:
            addDir('Favorites','url',4,FavoriteIcon,FavoriteFanart,'','','','')
        if addon.getSetting("browse_xml_database") == "true":
            addDir('XML Database','http://xbmcplus.xb.funpic.de/www-data/filesystem/',15,icon,FANART,'','','','')
        if addon.getSetting("browse_community") == "true":
            addDir('Community Files','community_files',16,icon,FANART,'','','','')
        if os.path.exists(history) == True and addon.getSetting('searchhistorymenu') == 'true':
            addDir('Search History','history',25,HistoryIcon,HistoryFanart,'','','','')
        if addon.getSetting("searchyt") == "true":
            addDir('Search:Youtube','youtube',25,YouTubeIcon,YouTubeFanart,'','','','')
        if addon.getSetting("searchDM") == "true":
            addDir('Search:dailymotion','dmotion',25,DailyMotionIcon,DailyMotionFanart,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification("+addon_name+",New source added.,5000,"+icon+")")
        if not url is None:
...
#                print channel
                ...
                if name.startswith('tmdb='):
                    ...
                    for items in BYBAPI.Details_list:
                        ...
                        tmdb_fanart = items.get('backdrop_path',FANART)
                ...
                if thumbnail == None:
                    if linkedUrl.startswith('#YTsearch'):
                        if _Edit.YouTubeIcon == '':
                            thumbnail = os.path.join(ART,'youtube.png')
                        else:
                            thumbnail = _Edit.YouTubeIcon
                    elif tmdbapi:
                        thumbnail = tmdb_icon
                    else:
                        thumbnail = ''
                ...
                try:
                    ...
                    elif linkedUrl.startswith('https://www.youtube.com/') and not 'playlist?list=' in linkedUrl:
                        from libs import youtube
                        youtube.AddChannel(name,linkedUrl,thumbnail,fanArt,desc,genre,date,credits)
                    elif linkedUrl.startswith('https://www.youtube.com/playlist?list='):
                        addDir(name.encode('utf-8'),linkedUrl.encode('utf-8'),34,thumbnail,fanArt,desc,genre,date,credits)
                    elif linkedUrl.startswith('#YTsearch'):
                        if '=' in linkedUrl:
                            ...
                            search_term = ''
                        addDir(name.encode('utf-8'),search_term,35,thumbnail,fanArt,desc,genre,date,credits)
                    else:
                        #print linkedUrl
                        addDir(name.encode('utf-8'),linkedUrl.encode('utf-8'),1,thumbnail,fanArt,desc,genre,date,None,'source')
                except:
                    koding.dolog('There was a problem adding directory from getData(): '+name.encode('utf-8', 'ignore'),line_info=True)
        else:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('%s Select A Source'%addon_name, List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('%s Searching Please wait'%addon_name,' ')
...
def getItems(items,fanart):
        ...
        for item in items:
            ...
            tmdbapi = False
            try:
                #tv show   tmdb=xxxxxx=s1_e3tvshow
                ...
                if name.startswith('tmdb='):
                    ...
                    if fullname.startswith('tmdb=') and fullname.endswith('tvshow'):
                        if addon.getSetting('season_episode_number') == 'true':
                ...
                if len(url) > 1:
                    ...
                    for i in url:
                        ...
                        if addon.getSetting('ask_playlist_items') == 'true':
                            ...
                            elif  (any(x in i for x in resolve_url)or host) and  i.startswith('http'):
                    ...
                    if isXMLSource:
                        ...
                        BYB.addDir_file(name.encode('utf-8'),'',00,thumbnail,fanArt,desc,'','','')				
                    else: 
                        addLink(url[0],name.encode('utf-8', 'ignore'),thumbnail,fanArt,desc,genre,date,True,None,regexs,total)
                    koding.dolog('url[0]=%s'%url[0],line_info=True)
                ...
                try:
                    ...
                    for i in reg_item:
                        ...
                        try:
                            ...
                            koding.dolog("Regex: -- Not a setcookie")
                        try:
                    ...
                    regexs = None
                    koding.dolog('regex Error: '+name.encode('utf-8', 'ignore'))
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(%s,Please enable Update Commonresolvers to Play in Settings. - ,10000)"%addon_name)

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        xbmc.executebuiltin("XBMC.Notification(%s,Using RESOLVE module.. - ,5000)"%addon_name)
        host = RESOLVE.HostedMediaFile(url)
        if host:
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('%s','Choose a location to save files.',15000,"+icon+")"%addon_name)
            addon.openSettings()
    ...
    if 'youtube' in site_name:
        ...
        for item in search_res:
            try:
            	...
            	koding.dolog( 'This item is ignored::',line_info=True)'''
...
if mode==None:
    koding.dolog("Index",line_info=True)
    SKindex()
    ...
    koding.dolog("get info then download")
    ytdl_download(url,name,'video') 
elif mode==24:
    koding.dolog("Audio only youtube download")
    ...
    koding.dolog("YouTube/DMotion From Search History")
    name = name.split(':')
    search(url,search_term=name[1])
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==27:
    koding.dolog("Using IMDB id to play in Pulsar")
    ...
    koding.dolog('search youtube search_term= %s'%url,line_info=True)
    if url == '':
    ...
    koding.dolog('mode +100'+str(mode),line_info=True)
    from libs import modes
    modes.Mode(mode)
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
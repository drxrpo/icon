...
def iVueInt():
	...
	dp.create("Durex [COLOR red]TV[/COLOR]","Copying ini",'', 'Please Wait')
	unzip(inizip,ivuecreate,dp)
	time.sleep(5)
	a = open(ivuecreateini).read()
	b = a.replace('<DUREX1UN>',username).replace('<DUREX1PW>',password)
	...
	iVueSet = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.durextv/resources/ivue','ivuesettings.xml'))
	iVueFold = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.durextv/resources/ivue'))
	...
	dp = xbmcgui.DialogProgress()
	dp.create("Durex [COLOR red]TV[/COLOR]","Copying DB",'', 'Please Wait')
...
def install(name,url):
    ...
    dp = xbmcgui.DialogProgress()
    dp.create("Durex [COLOR red]TV[/COLOR]","Installing iVue TV Guide...",'', 'Please Wait')

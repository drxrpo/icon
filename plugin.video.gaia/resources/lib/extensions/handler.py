...
class HandleElementum(Handle):
	...
	def handle(self, link, item, download = False, popups = False, close = True):
		...
		'''result = network.Networker(tools.Elementum.linkPlay()).request(form = [
			{'name' : 'uri', 'data' : link},
			{'name' : 'file', 'data' : 'null'},
		])
		if result == None: interface.Dialog.notification(title = 35316, message = 35430, icon = interface.Dialog.IconError, time = 10000)
		'''

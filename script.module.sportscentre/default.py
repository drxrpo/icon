...
addon_id            = 'script.module.sportscentre'
AddonTitle          = '[COLOR orange]StarTec Sports Center[/COLOR]'
...
baseurl             = 'http://gen-tec.co/gentecwiz/PLEASE%20SUBSCRIBE%20TO%20CHARLIE%20TEC%20YOUTUBE/star/Sports/Sportshub.xml'

######### baseurl             = 'https://pastebin.com/raw/DfyrkZf6'
...
GET_REPO_VERSION    = xbmc.translatePath('special://home/addons/repository.Gentec/addon.xml')
...
REPO_FOLDER         = xbmc.translatePath(os.path.join('special://home/addons/repository.Gentec'))
...
def ENABLE_DB_ADDON(id):
    ...
    try:
        ...
        dialog.ok(AddonTitle, id + ' has been enabled!', 'Thank you for using Sports Zone.')
...
def GetMenu():
    ...
    ##############################addLink(kodi.giveColor(kodi.countGitHubIssues('https://github.com/Colossal1/script.module.sportscentre/issues'),'blue',True) + kodi.giveColor(' | Click To View Issues','white',True),'null',994,'https://imgur.com/kg2XOOc',fanarts,'')
...
def REDDIT_MAIN():
    ...
    addDir('Click for Sports Zone Recommended Reddits','url',322,icon,fanarts)
...
def PLAYLINK(name,url,iconimage):
    ...
    if not "plugin" in url:
        try:
            ...
            dialog.ok(AddonTitle, "[COLOR blue]Sorry there was a problem playing this link.[/COLOR]","[COLOR blue]Sports Zone has plenty of content to choose from :-D[/COLOR]")
            quit()
...
def GITHUB_ISSUES():
    ...
    githubissues.run('Colossal1/script.module.sportscentre', '%s' % name)
    ...
    if (not items) or (len(items) < 1):
        msg_text = kodi.giveColor('No %s issues with Sports Zone at this time.' % name.title(),'blue',True)
    else:
        msg_text = kodi.giveColor('%s Issues with Sports Zone\n' % name.title(),'blue',True) + kodi.giveColor('Report Issues @ https://github.com/Colossal1/script.module.sportscentre/issues','white',True) + '\n---------------------------------\n\n'
        for item in items:
            ...
            msg_text += '[B]ID: %s | Label: %s \nBy: %s on %s at %s[/B] \n\nTitle: %s \nMessage %s \n\n---------------------------------\n\n' \
                         % (id, \
                            ...
                            title, \
                            body)
    TEXTBOXES_GIT('Sports Zone Issues', msg_text)

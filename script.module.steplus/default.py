import base64, hashlib, os, random, re, requests, shutil, string, sys, urllib, urllib2, json, urlresolver, ssl, zipfile, \
    urlparse
...
kidsurl = base64.b64decode(
    'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3NDbGFya2VJc0JhY2svU3RyZWFtSHViL21hc3Rlci9MaW5rcy9LaWRzL2tpZHNjb3JuZXIueG1s')
docurl = 'http://documentaryheaven.com'
mov2 = 'http://zmovies.to'
wwe = 'http://watchwrestling.in'
tv = base64.b64decode(
    'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3NDbGFya2VJc0JhY2svU3RyZWFtSHViL21hc3Rlci9MaW5rcy8yNDcvMjQ3dHYueG1s')
...
def fullmatchtv(url):
    ...
    all = re.compile('<div class="td-module-thumb">.+?href="(.+?)".+?title="(.+?)".+?src="(.+?)"', re.DOTALL).findall(
        part)
...
def MOV2CAT():
    addDir('[COLOR red]L[/COLOR]atest Releases', 'NEW:https://www.filmissimi.net/genere/animazione', 79, icon, fanart,
           '')
    addDir('[COLOR red]M[/COLOR]ost Popular', 'https://www.filmissimi.net/genere/horror', 79, icon, fanart, '')
...
def TVREQUESTCAT():
    addDir('Everybody Loves Raymond', 'ELR', 50, 'http://www.gstatic.com/tv/thumb/tvbanners/184243/p184243_b_v8_ab.jpg',
           '', '')
    addDir('How i Met Your Mother', 'HIMYM', 50,
           'http://www.gstatic.com/tv/thumb/tvbanners/9916255/p9916255_b_v8_aa.jpg', '', '')
...
def MUSICCOL():
    addDir('BBC Radio 1 Live Lounge Collection', 'https://www.discogs.com/label/804379-Radio-1s-Live-Lounge', 70, icon,
           fanart, '')
...
def xxx(url):
    ...
    try:
        ...
        pass
    regex = r"class=\"thumb-block \">.*?data-src=\"([^\"]+)\".*?<a href=\"([^\"]+)\".*?>(.*?)</a></p>"
    all_videos = re.compile(regex, re.MULTILINE).findall(link)
    for thumb, url, name in all_videos:
...
def xxxgenre(url):
    ...
    #			return
    link = OPEN_URL(url)
    main = regex_from_to(link, '<ul class=\"tags-list\" id=\"tags\">', '</ul>')
...
def documentary(url):
    ...
    for a in regex:
        url = regex_from_to(a, '="', '"')
        title = regex_from_to(a, '">', '<').replace("&amp;", "&").replace('&#39;', "'").replace('&quot;', '"').replace(
...
def resolvetwentyfourseven(url, name):
    ...
    liz.setProperty('IsPlayable', 'true')
    liz.setPath(
        m3u + '|User-Agent=Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.36&Referer=' + ref)
    xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, liz)


def home():
    home = xbmc.executebuiltin('XBMC.RunAddon(plugin://plugin.video.Stefano/?action=)')
    return home
...
def toongetresolve(name, url):
    ...
    try:
        ...
        xbmcgui.Dialog().notification('[COLOR red][B]Stefano Plus[/B][/COLOR]', 'Oops, This Link Is Down!')
...
def bbcmusicindex(url):
    ...
    if 'singles' in url:
        for a in all:
            ...
            icon = regex_from_to(a, '         src="', '"')
            url = 'http://woodmp3.net/mp3.php?q=' + (name.translate(None, '\/:*?"\'<>|!,')).replace(' ', '-').replace(
                '--', '-').lower()
...
def urlperosnal():
    open = OPEN_URL('https://raw.githubusercontent.com/adrianhn/adryanlist/master/list.xml')
    all = regex_get_all(open, '<item>', '</item>')
    for a in all:
        name = regex_from_to(a, '<title>', '</title>')
        url = regex_from_to(a, '<externallink>', '</externallink>')
        icon = regex_from_to(a, '<thumbnail>', '</thumbnail>')
        addDir(name, url, 999, icon, fanart, '')
...
def UKNowMusic(url):
    ...
    for a in all:
        ...
        if not 'DVD' in title:
            xbmc.log(str(url))
            addDir('[COLOR red]%s[/COLOR] | [COLOR red]%s[/COLOR]' % (country, year) + ' | ' + title, url, 71, icon,
                   fanart, desc)
...
def discogindex(url):
    open = OPEN_URL(url)
    log(open)
    all = re.compile('"description".+?title".+?"(.+?)".+?thumbnail".+?"(.+?)".+?file.+?id".+?"(.+?)"',
                     re.DOTALL | re.MULTILINE).findall(open)
...
def addDir(name, url, mode, iconimage, fanart, description):
    u = sys.argv[0] + "?url=" + url + "&mode=" + str(mode) + "&name=" + urllib.quote_plus(
        name) + "&iconimage=" + urllib.quote_plus(iconimage) + "&description=" + urllib.quote_plus(description)
    ok = True
    liz = xbmcgui.ListItem(name, iconImage="DefaultFolder.png", thumbnailImage=iconimage)
    liz.setInfo(type="Video", infoLabels={"Title": name, "Plot": description})
...
def playf4m(url, name):
    ...
    f4mProxyHelper().playF4mLink(url, name, proxy, proxy_use_chunks, maxbitrate, simpleDownloader, auth_string,
                                 streamtype, False, swf)


def showpremiumimage():
    premium_jpg = xbmc.translatePath(
        os.path.join('special://home/addons/script.module.steplus/resources/premium', 'premium_image.jpg'))
...
xbmcaddon.Addon().setSetting('xxxpass', 'manovetrata')
xbmcaddon.Addon().setSetting('enablexxxpass', 'true')
xbmcaddon.Addon().setSetting('freshstart', 'false')
...
# OpenELEQ: query & type-parameter (added to line above)
    ...
    xbmc.executebuiltin("ActivateWindow(Videos,plugin://plugin.video.Stefano/?title=postupdate)")

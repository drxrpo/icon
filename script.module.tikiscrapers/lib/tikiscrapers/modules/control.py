...
lang = xbmcaddon.Addon("script.module.tikiscrapers").getLocalizedString

lang2 = xbmc.getLocalizedString

setting = xbmcaddon.Addon("script.module.tikiscrapers").getSetting

setSetting = xbmcaddon.Addon("script.module.tikiscrapers").setSetting
...
addonInfo = xbmcaddon.Addon("script.module.tikiscrapers").getAddonInfo
...
def multiSelectDialog(string, dialog_list, function_list=None, preselect= []):
...
def openSettings(query):
    try:
        ...
        execute('SetFocus(%i)' % (int(menu) - button))
        execute('SetFocus(%i)' % (int(function) - control))
    except: return
...
def busy():
    if kodiVersion() >= 18: return execute('ActivateWindow(busydialognocancel)')
    else: return execute('ActivateWindow(busydialog)')

def idle():
    if kodiVersion() >= 18: return execute('Dialog.Close(busydialognocancel)')

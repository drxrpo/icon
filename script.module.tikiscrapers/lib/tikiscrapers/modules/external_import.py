...
class ExternalImporter():
    def __init__(self):
        ...
        ('script.module.13clowns', 'from resources.lib.', ['lib', 'resources', 'lib', 'sources']),
    ...
    def removeExternal(self):
        try:
            ...
            removeFolder = xbmc.translatePath(os.path.join(control.addonInfo('path'), 'lib', 'tikiscrapers', removeModule + '/'))
            ...
            if control.setting('module.provider').lower() == moduleChoice.split('.')[-1]:
                control.setSetting('module.provider', 'Tikiscrapers')
    ...
    def importBackup(self, backupPath, module):
        destPath = xbmc.translatePath(os.path.join(control.addonInfo('path'), 'lib', 'tikiscrapers', module + '/'))
    ...
    def setImportAddon(self):
        ...
        currentImports = [i.lower() for i in sorted(providerSources()) if not i.lower() == 'tikiscrapers']
        ...
        self.destPath = xbmc.translatePath(os.path.join(control.addonInfo('path'), 'lib', 'tikiscrapers', 'sources_%s' % self.sourcesFolder + '/'))
    ...
    def _removeGarbage(self, path):
        try:
            ...
            for directory in directories:
                ...
                if end_directory.startswith(('.', 'de', 'es', 'gr', 'pl', 'en_direct')):
                    xbmcvfs.rmdir(directory + '/', True)
        except: pass

    def _instancesRename(self, path):
        replacements = [['%s' % self.replacement, 'from tikiscrapers.']]
        directories, files = self._listDirectory(path, absolute=True)
        for file in files:
            if file.endswith('.py'):

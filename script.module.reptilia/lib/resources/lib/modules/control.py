...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.reptilia.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.reptilia.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.reptilia.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.reptilia.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.reptilia.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.reptilia.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')

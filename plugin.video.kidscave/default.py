# -*- coding: utf-8 -*-
"""
    default.py --- kidscave Addon entry point
...
import base64
import __builtin__
root_xml_url = base64.b64decode(b"aHR0cDovL3d3dy5ndGFuZHJvaWQuYW5hcmNoeW1lZGlhcmV2b2x1dGlvbi5jb20vcm9ndWUva2lkc2NhdmUueG1s")
...
from resources.lib.util.xml import kidscaveList, display_list
...
def get_list_uncached(url):
    """display kidscave list uncached"""
    global content_type
    kidscave_list = kidscaveList(url, cached=False)
    if not kidscave_list:
        koding.dolog(_("returned empty for ") + url)
    items = kidscave_list.get_list()
    content = kidscave_list.get_content_type()
    if items == []:
        return False
    if content:
...
def get_list(url):
    """display kidscave list"""
    global content_type
    kidscave_list = kidscaveList(url)
    if not kidscave_list:
        koding.dolog(_("returned empty for ") + url)
    items = kidscave_list.get_list()
    content = kidscave_list.get_content_type()
...
def all_episodes(url):
    ...
    for index, season_url in enumerate(season_urls):
        ...
        kidscave_list = kidscaveList(season_url)
        result_items.extend(kidscave_list.get_list(skip_dialog=True))
...
if foldername in ["", "plugin.program.super.favourites"]:
    __builtin__.kidscave_WIDGET = True
else:
    __builtin__.kidscave_WIDGET = False

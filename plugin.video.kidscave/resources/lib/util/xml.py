# -*- coding: utf-8 -*-
"""
    xml.py --- functions dealing with kidscave xml list format
...
class kidscaveList(object):
    """
wrapper class for kidscave list functions
    """
    ...
    def __init__(self, url, cached=True):
        ...
        if xml:
            self.xml = xml
            self.content = ""
            self.list = kidscaveList.list_pattern.findall(xml)
            info = kidscaveItem(xml.split('<item>')[0].split('<dir>')[0])
    ...
    def get_list(self, input_list=None, skip_meta=False, skip_dialog=False):
        ...
        Returns:
        list of kidscave items
        """
        items = []
        meta = ADDON.getSetting("metadata") == "true"
        try:
            is_widget = __builtin__.kidscave_WIDGET
        except:
            is_widget = False
        if is_widget:
            skip_dialog = True
...
class kidscaveItem(object):
    """represents an item in a kidscave xml list"""

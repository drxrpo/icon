...
from resources.lib.util.xml import kidscaveItem, kidscaveList
...
from resources.lib.player import kidscavePlayer
...
def choose_quality(link, name=None, selected_link=None):
    ...
    Keyword Arguments:
    link -- kidscaveitem link with sublinks
    ...
    if link.startswith("http") or link.startswith("plugin"):
        ...
        kidscave_link = kidscaveItem(link)
        sublinks = kidscave_link.getAll("sublink")
        if not sublinks:
            sublinks = [kidscave_link]
    links = []
...
def get_sources(item):
    ...
    Keyword Arguments:
    item -- kidscaveItem to try playing
    """
    ...
    if item.startswith("<plugin>"):
        # link to plugin
        link = kidscaveItem(item)["link"]
        sublinks = kidscaveItem(link).getAll("sublink")
        if sublinks:
            if len(sublinks) > 1:
        ...
        return
    item = kidscaveItem(item)

    link = item["link"]
    if not link or link.replace("\t", "") == "":
        return
    meta = kidscaveItem(item["meta"])
    title = meta["title"]
    ...
    kidscaveplayer = kidscavePlayer(resume=False)
    try:
        spec = {
            "identifier": imdb,
            "season": season or "0",
        ...
        if match:
            ...
            if match["currentTime"] and not match["currentTime"] == "0":
                if dialog.yesno(ADDON.getAddonInfo("name"),
                                ...
                                yeslabel=_("Resume"),
                                nolabel=_("Restart")):
                    kidscaveplayer = kidscavePlayer(resume=True)
    except:
        pass

    kidscaveplayer.setItem(item)
...
def queue_source(item, depth=0):
    ...
    kidscave_item = kidscaveItem(item)
    playlist = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
    if "<item>" in str(kidscave_item):
        play = False
        ...
        playlist.add(
            ...
            xbmcgui.ListItem(
                kidscave_item["title"], iconImage=kidscave_item.get("thumbnail", "")))
        if play:
            play_queue()
    else:
        link = kidscave_item.get("url", kidscave_item.get("link", ""))
        kidscavelist = kidscaveList(link).get_raw_list()
        for list_item in kidscavelist:

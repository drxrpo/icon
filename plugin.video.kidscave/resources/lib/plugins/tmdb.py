"""
    tmdb.py --- kidscave Plugin for accessing tmdb data
...
from resources.lib.util.xml import kidscaveItem, kidscaveList, display_list
...
def tmdb(url):
    ...
    kidscavelist = kidscaveList(xml)
    display_list(kidscavelist.get_list(), __builtin__.content_type)
...
def tmdb_tv_show(url):
    ...
    if not xml:
        ...
        save_to_db(xml, url)
    kidscavelist = kidscaveList(xml)
    display_list(kidscavelist.get_list(), kidscavelist.get_content_type())

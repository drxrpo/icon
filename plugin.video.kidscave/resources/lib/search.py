...
"""
    seach.py --- functions dealing with searching kidscave
...
if theme and theme != 'DEFAULT' and theme != 'none':
    fanart = kidscaveList.set_theme(theme)
else:
...
def do_search(term=None):
    ...
    if result:
        ...
        return
    kidscavelist = kidscaveList("")
    kidscavelist.list_image = xbmcaddon.Addon().getAddonInfo('icon')
    theme = xbmcaddon.Addon().getSetting('theme')
    if theme and theme != 'DEFAULT' and theme != 'none':
        kidscavelist.list_fanart = kidscavelist.set_theme(theme)
    else:
        kidscavelist.list_fanart = xbmcaddon.Addon().getAddonInfo('fanart')
    result_list = []
    exact_result_list = []
    ...
    for result in results:
        item = kidscavelist.process_item(result["item"])

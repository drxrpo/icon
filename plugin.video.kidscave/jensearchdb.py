...
def kidscave_get_tag_content(collection, tag, default):
    try:
...
def kidscave_list(content):
    ...
    vip = kidscave_get_tag_content(info, 'poster', '0')
    image = kidscave_get_tag_content(info, 'thumbnail', '0')
    fanart = kidscave_get_tag_content(info, 'fanart', '0')
    ...
    for item in items:
        try:
            ...
            date = kidscave_get_tag_content(item, 'date', '')
            if re.search(r'\d+', date):
                name += ' [COLOR red] Updated %s[/COLOR]' % date
            meta = kidscave_get_tag_content(item, 'meta', '0')
            url = kidscave_get_tag_content(item, 'link', '0')
            ...
            image2 = kidscave_get_tag_content(item, 'thumbnail', image)
            if not str(image2).lower().startswith('http'):
                image2 = '0'

            fanart2 = kidscave_get_tag_content(item, 'fanart', fanart)
            if not str(fanart2).lower().startswith('http'):
                fanart2 = '0'

            content = kidscave_get_tag_content(meta, 'content', '0')

            if content in ['episode', 'season']:
                continue

            imdb = kidscave_get_tag_content(meta, 'imdb', '0')
            tvdb = kidscave_get_tag_content(meta, 'tvdb', '0')
            tvshowtitle = kidscave_get_tag_content(meta, 'tvshowtitle', '0')
            title = kidscave_get_tag_content(meta, 'title', '0')
            if title == '0' and not tvshowtitle == '0':
                title = tvshowtitle

            year = kidscave_get_tag_content(meta, 'year', '0')
            premiered = kidscave_get_tag_content(meta, 'premiered', '0')
            season = kidscave_get_tag_content(meta, 'season', '0')
            episode = kidscave_get_tag_content(meta, 'episode', '0')
...
def main():
    ...
    sections = [
        {'name': 'kidscave', # can change kidscave to whatever you want.  This wil name the output xml kidscave.xml
         'url': "http://yoururlforthesectionyouwant.xml", # your section xml goes here
         'poster': 'kidscave'}, # can change kidscave to whatever you want. This will be displayed as the poster name on kidscave.xml and
                           # also in the actual search in your addon in kodi eg kidscave - The Avengers
    ]

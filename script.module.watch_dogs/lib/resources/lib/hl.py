...
class HostedLink:
    def __init__(self, title, year, imdb=None, tvdb=None, host=None, include_disabled=False, timeout=30, exclude=None, enable_debrid=False):
        ...
        xbmcvfs.mkdir(xbmc.translatePath(xbmcaddon.Addon("script.module.watch_dogs").getAddonInfo('profile')))
    ...
    def get_url(scraper, title, show_year, year, season, episode, imdb, tvdb, type, cache_location, maximum_age, check_url = False, debrid = False):
        cache_enabled = xbmcaddon.Addon('script.module.watch_dogs').getSetting("cache_enabled") == 'true'
        try:
            ...
            if check_url:
                ...
                for source in sources:
                    if source["direct"]:
                        ...
                        try:
                            hmf = resolveurl.HostedMediaFile(url=source['url'], include_disabled=False,

...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.it.artwork)'):
        return os.path.join(
            xbmcaddon.Addon('script.it.artwork').getAddonInfo('path'),
            'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.it.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.it.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.it.metadata)'):
        return os.path.join(
            xbmcaddon.Addon('script.it.metadata').getAddonInfo('path'),

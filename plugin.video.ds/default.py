...
def root():
    ...
    if enable_newswin == 'true':
        koding.Add_Dir(name='[B][COLOR lightseagreen]L[COLOR white]atest [COLOR lightseagreen]N[COLOR white]ews [COLOR lightseagreen]A[COLOR white]nd [COLOR lightseagreen]U[COLOR white]pdates[/COLOR][/B]', url='{"my_text":"[B][COLOR lightseagreen]Latest News[/COLOR][/B][CR]!!!","my_desc":""}', mode='dialog_example', folder=False, icon=os.path.join(art_path,'icon.png'), fanart=os.path.join(art_path,'fanart.jpg'))

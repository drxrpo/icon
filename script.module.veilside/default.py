...
addon_id   = 'script.module.veilside'
...
def NOVAMOVIERESOLVE(url):
	...
	try:
		for a in match:
			quality = '%s' %regex_from_to(a,'label": "','"')
			url     =  regex_from_to(a,': "','"')
			if not '.srt' in url:
...
def putlockerhdread(url):
        ...
        try:
                ...
                for url, pn in match:
                        ...
                        addDir('Page %s ' %pn,url,19,icon,fanart,'')
        except: pass
...
def xxxadd_next_button(link):
			try:
				if '/tags/' in link:
					...
					addDir('Next Page>>>',xxxurl+nextp,24,'','','')
			except: pass
...
def documentary(url):
	...
	try:
		...
		if link == "":
			...
			addDir('NEXT PAGE',link,35,thumb,fanart,'')
	except:pass
def resolvedoc(url):
	open = OPEN_URL(url)
	xbmc.log(str(open))
...
def home():
	home = xbmc.executebuiltin('XBMC.RunAddon(plugin://plugin.video.veilside/?action=)')
	return home
...
def toongetresolve(name,url):
    ...
    try:
		...
		xbmcgui.Dialog().notification('','Oops, This Link Is Down!')
...
def showpremiumimage():
	premium_jpg = xbmc.translatePath(os.path.join('special://home/addons/script.module.veilside/resources/premium', 'premium_image.jpg'))
...
# OpenELEQ: query & type-parameter (added to line above)
	...
	xbmc.executebuiltin('XBMC.RunScript(script.module.veilside)')
	...
	xbmc.executebuiltin('Addon.OpenSettings(plugin.video.veilside)')
	sys.exit()
	...
	if xbmcaddon.Addon('plugin.video.veilside').getSetting('Username') == "":
		d = xbmcgui.Dialog().yesno('','Would Like To Log-In?')
		if not d:
			showpremiumimage()
			sys.exit()
		else:
			from resources.premium import premium
			d = xbmcgui.Dialog().yesno('','You will need to enter your Login details in the Addons Settings','Would you like us to open the settings for you now?')
			if d:
				xbmcaddon.Addon('plugin.video.veilside').openSettings()
				premium.start('NEW')
			else:
				sys.exit()
	else:

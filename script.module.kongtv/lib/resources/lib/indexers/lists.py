...
from resources.lib.modules import cache,client,control,metacache,trailer
from resources.lib.modules import regex,views,workers,youtube
...
class indexer:
    ...
    def rootMC(self):
        try:
            regex.clear()
            url = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L3Y5enVRUTFN'.decode('base64')
    ...
    def root(self):
        try:
            regex.clear()
            url = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L3lTQ1RxcWFO'.decode('base64')
            self.list = self.noname_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootXXX(self):
        try:
            regex.clear()
            url = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L1JSYjhzTnBF'.decode('base64')
            self.list = self.noname_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def noname_list(self, url, result=None):
        try:
            ...
            api_data = client.request(base64.b64decode('aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L2s5MmhDdnVZ'), timeout='5')

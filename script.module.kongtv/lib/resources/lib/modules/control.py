...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.kongtv.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.kongtv.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.kongtv.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.kongtv.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.kongtv.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.kongtv.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')

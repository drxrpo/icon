...
INCLUDEBENELLI    = wiz.getS('includebenelli')
INCLUDELOKI    = wiz.getS('includeloki')
INCLUDEYODA    = wiz.getS('includeyoda')
INCLUDEREPTILIA    = wiz.getS('includereptilia')
...
ICONFORCE     = uservar.ICONFORCE if not uservar.ICONFORCE == 'http://' else ICON
ICONRD     = uservar.ICONRD if not uservar.ICONRD == 'http://' else ICON
ICONSET     = uservar.ICONSET if not uservar.ICONSET == 'http://' else ICON
...
def rdsetup():	
	...
	if not xbmc.getCondVisibility('System.HasAddon(script.module.urlresolver)'): 
		xbmc.executebuiltin("InstallAddon(script.module.urlresolver)")
	addFile ('[COLOR red]ResolverUrl[/COLOR] [COLOR gold]Real-Debrid Authorization[/COLOR]','resolveurl', fanart=FANART,icon=ICONRD, themeit=THEME2)
	addFile ('[COLOR blue]URLResolver[/COLOR] [COLOR gold]Real-Debrid Authorization[/COLOR]','urlresolver', fanart=FANART,icon=ICONRD, themeit=THEME2)
	setView('files', 'viewType')
...
def maintMenu(view=None):
	...
	if includeall == 'true':
		...
		includeres = 'true' if INCLUDERESOLVE  == 'true' else 'false'
		includepla = 'true' if INCLUDEPLACENTA  == 'true' else 'false'
		includenep = 'true' if INCLUDENEPTUNE  == 'true' else 'false'
		includereb = 'true' if INCLUDEGENESISREBORN  == 'true' else 'false'
		includefli = 'true' if INCLUDEFLIXNET  == 'true' else 'false'
		includeura = 'true' if INCLUDEURANUS  == 'true' else 'false'
		includenum = 'true' if INCLUDENUMBERS  == 'true' else 'false'
		includekra = 'true' if INCLUDEKRATOS  == 'true' else 'false'
		includeinc = 'true' if INCLUDEINCURSION  == 'true' else 'false'
		includeben = 'true' if INCLUDEBENELLI  == 'true' else 'false'
		includelok = 'true' if INCLUDELOKI  == 'true' else 'false'
		includeyod = 'true' if INCLUDEYODA  == 'true' else 'false'
		includerep = 'true' if INCLUDEREPTILIA  == 'true' else 'false'
	...
	if view == "addon" or SHOWMAINT == 'true': 
		...
		addFile('Force Update Addons',            'forceupdate2',     icon=ICONFORCE, themeit=THEME3)
	...
	if includevid == 'true':
		...
		addFile('--- Include Resolve Url: %s' % includeres.replace('true',on).replace('false',off), 'togglecache', 'includeresolve', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('--- Include Genesis Reborn: %s' % includereb.replace('true',on).replace('false',off), 'togglecache', 'includegenesisreborn', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('--- Include Flixnet: %s' % includefli.replace('true',on).replace('false',off), 'togglecache', 'includeflixnet', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('--- Include Numbers: %s' % includeura.replace('true',on).replace('false',off), 'togglecache', 'includenumbers', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Kratos: %s' % includeura.replace('true',on).replace('false',off), 'togglecache', 'includekratos', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Incursion: %s' % includeura.replace('true',on).replace('false',off), 'togglecache', 'includeincursion', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Benelli: %s' % includeura.replace('true',on).replace('false',off), 'togglecache', 'includebenelli', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Loki: %s' % includeura.replace('true',on).replace('false',off), 'togglecache', 'includeloki', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Yoda: %s' % includeura.replace('true',on).replace('false',off), 'togglecache', 'includeyoda', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Reptilia: %s' % includeura.replace('true',on).replace('false',off), 'togglecache', 'includereptilia', icon=ICONMAINT, themeit=THEME3)
...
def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includenan', 'includeurl', 'includebobunleashed', 'includeelysium', 'includecovenant', 'includebob', 'includephoenix', 'includespecto', 'includegenesis', 'includeexodus', 'includeonechan', 'includesalts', 'includesaltslite', 'includeresolve', 'includeplacenta', 'includeneptune', 'includegenesisreborn', 'includeflixnet', 'includeuranus']
	titlelist = ['Include Video Addons', 'Include All Addons','Include NaN Scrapers', 'Include Url Resolver', 'Include Bob Unleashed', 'Include Elysium', 'Include Covenant', 'Include Bob', 'Include Phoenix', 'Include Specto', 'Include Genesis', 'Include Exodus', 'Include One Channel', 'Include Salts', 'Include Salts Lite HD', 'Include Resolve Url', 'Include Placenta', 'Include Neptune', 'Include Genesis Reborn', 'Include Flixnet', 'Include Uranus']
...
elif mode=='forceupdate2'    : wiz.forceUpdate2()

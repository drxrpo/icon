# -*- coding: utf-8 -*-
"""
    xml.py --- functions dealing with totally xml list format
...
class totallyList(object):
    """
wrapper class for totally list functions
    """
    ...
    def __init__(self, url, cached=True):
        ...
        if xml:
            self.xml = xml
            self.content = ""
            self.list = totallyList.list_pattern.findall(xml)
            info = totallyItem(xml.split('<item>')[0].split('<dir>')[0])
    ...
    def get_list(self, input_list=None, skip_meta=False, skip_dialog=False):
        ...
        Returns:
        list of totally items
        """
        items = []
        meta = ADDON.getSetting("metadata") == "true"
        try:
            is_widget = __builtin__.totally_WIDGET
        except:
            is_widget = False
        if is_widget:
            skip_dialog = True
...
class totallyItem(object):
    """represents an item in a totally xml list"""

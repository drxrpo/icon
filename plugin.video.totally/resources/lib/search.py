...
"""
    seach.py --- functions dealing with searching totally
...
if theme and theme != 'DEFAULT' and theme != 'none':
    fanart = totallyList.set_theme(theme)
else:
...
def do_search(term=None):
    ...
    if result:
        ...
        return
    totallylist = totallyList("")
    totallylist.list_image = xbmcaddon.Addon().getAddonInfo('icon')
    theme = xbmcaddon.Addon().getSetting('theme')
    if theme and theme != 'DEFAULT' and theme != 'none':
        totallylist.list_fanart = totallylist.set_theme(theme)
    else:
        totallylist.list_fanart = xbmcaddon.Addon().getAddonInfo('fanart')
    result_list = []
    exact_result_list = []
    ...
    for result in results:
        item = totallylist.process_item(result["item"])

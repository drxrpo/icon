...
from resources.lib.util.xml import totallyItem, totallyList
...
from resources.lib.player import totallyPlayer
...
def choose_quality(link, name=None, selected_link=None):
    ...
    Keyword Arguments:
    link -- totallyitem link with sublinks
    ...
    if link.startswith("http") or link.startswith("plugin"):
        ...
        totally_link = totallyItem(link)
        sublinks = totally_link.getAll("sublink")
        if not sublinks:
            sublinks = [totally_link]
    links = []
...
def get_sources(item):
    ...
    Keyword Arguments:
    item -- totallyItem to try playing
    """
    ...
    if item.startswith("<plugin>"):
        # link to plugin
        link = totallyItem(item)["link"]
        sublinks = totallyItem(link).getAll("sublink")
        if sublinks:
            if len(sublinks) > 1:
        ...
        return
    item = totallyItem(item)

    link = item["link"]
    if not link or link.replace("\t", "") == "":
        return
    meta = totallyItem(item["meta"])
    title = meta["title"]
    ...
    totallyplayer = totallyPlayer(resume=False)
    try:
        spec = {
            "identifier": imdb,
            "season": season or "0",
        ...
        if match:
            ...
            if match["currentTime"] and not match["currentTime"] == "0":
                if dialog.yesno(ADDON.getAddonInfo("name"),
                                ...
                                yeslabel=_("Resume"),
                                nolabel=_("Restart")):
                    totallyplayer = totallyPlayer(resume=True)
    except:
        pass

    totallyplayer.setItem(item)
...
def queue_source(item, depth=0):
    ...
    totally_item = totallyItem(item)
    playlist = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
    if "<item>" in str(totally_item):
        play = False
        ...
        playlist.add(
            ...
            xbmcgui.ListItem(
                totally_item["title"], iconImage=totally_item.get("thumbnail", "")))
        if play:
            play_queue()
    else:
        link = totally_item.get("url", totally_item.get("link", ""))
        totallylist = totallyList(link).get_raw_list()
        for list_item in totallylist:

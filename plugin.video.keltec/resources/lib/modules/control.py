...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.addons.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.addons.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.addons.artwork)') else setting('appearance.alt')
    if appearance == "keltec": appearance = "Default"
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.addons.artwork)')

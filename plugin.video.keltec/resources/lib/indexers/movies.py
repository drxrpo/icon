...
class movies:
        def __init__(self):
                ...
                if self.tmdb_key == '' or self.tmdb_key == None:
                        self.tmdb_key         = base64.b64decode('YzdjZDA1NTJiY2VhMzRlMjQzY2E2YTlmNjIyMGI4Njc=')
        ...
        def tmdb_custom_list(self, url):
                # print ("keltec LISTS", url)
                try:
                        result = client.request(url)
                        result = json.loads(result)
                        items = result['items']
                        # print ("keltec LISTS", items)
                except:
                        return
                next = ''
                for item in items:
                        try:
                                ...
                                title = client.replaceHTMLCodes(title)
                                title = title.encode('utf-8')
                                # print ("keltec LISTS", title)
                                year = item['release_date']
                                year = re.compile('(\d{4})').findall(year)[-1]
                                year = year.encode('utf-8')
                                # print ("keltec LISTS", year)
                                tmdb = item['id']
                                tmdb = re.sub('[^0-9]', '', str(tmdb))
                                tmdb = tmdb.encode('utf-8')
                                # print ("keltec LISTS", tmdb)
                                poster = item['poster_path']
                                if poster == '' or poster == None: raise Exception()
                                else: poster = '%s%s' % (self.tmdb_poster, poster)
                                poster = poster.encode('utf-8')
                                # print ("keltec LISTS", poster)
                                fanart = item['backdrop_path']
                                if fanart == '' or fanart == None: fanart = '0'
                                if not fanart == '0': fanart = '%s%s' % (self.tmdb_image, fanart)
                                fanart = fanart.encode('utf-8')
                                # print ("keltec LISTS", fanart)
                                premiered = item['release_date']
                                try: premiered = re.compile('(\d{4}-\d{2}-\d{2})').findall(premiered)[0]
                                except: premiered = '0'
                                premiered = premiered.encode('utf-8')
                                # print ("keltec LISTS", premiered)
                                rating = str(item['vote_average'])
                                if rating == '' or rating == None: rating = '0'
                                rating = rating.encode('utf-8')
                                # print ("keltec LISTS", rating)
                                votes = str(item['vote_count'])
                                try: votes = str(format(int(votes),',d'))
                                except: pass
                                if votes == '' or votes == None: votes = '0'
                                votes = votes.encode('utf-8')
                                # print ("keltec LISTS", votes)
                                plot = item['overview']
                                if plot == '' or plot == None: plot = '0'
                                plot = client.replaceHTMLCodes(plot)
                                plot = plot.encode('utf-8')
                                # print ("keltec LISTS", plot)
                                tagline = re.compile('[.!?][\s]{1,2}(?=[A-Z])').split(plot)[0]
                                try: tagline = tagline.encode('utf-8')
                                except: pass
                                # print ("keltec LISTS", tagline)
                                self.list.append({'title': title, 'originaltitle': title, 'year': year, 'premiered': premiered, 'studio': '0', 'genre': '0', 'duration': '0', 'rating': rating, 'votes': votes, 'mpaa': '0', 'director': '0', 'writer': '0', 'cast': '0', 'plot': plot, 'tagline': tagline, 'code': '0', 'imdb': '0', 'tmdb': tmdb, 'tvdb': '0', 'poster': poster, 'banner': '0', 'fanart': fanart, 'next': next})
                        except:
                                pass
                return self.list
        ...
        def movieDirectory(self, items):
                ...
                for i in items:
                        try:
                                ...
                                if "tt" in imdb                     : sysmetalliq = "plugin://plugin.video.metalliq/movies/add_to_library_parsed/imdb/%s/direct.keltec.q" % imdb
                                elif not tmdb == "0" or tmdb == None: sysmetalliq = "plugin://plugin.video.metalliq/movies/add_to_library_parsed/tmdb/%s/direct.keltec.q" % tmdb
                                else                                : sysmetalliq = "0"

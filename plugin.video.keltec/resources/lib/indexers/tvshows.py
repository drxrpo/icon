...
class tvshows:
    def __init__(self):
        ...
        self.tvdb_key             = base64.urlsafe_b64decode('QjE4N0JFRDEwMzRCOEMxNw==')
        self.datetime             = (datetime.datetime.utcnow() - datetime.timedelta(hours = 5))
        self.trakt_user           = control.setting('trakt.user').strip()
        self.imdb_user            = control.setting('imdb.user').replace('ur', '')
        self.lang                 = control.apiLanguage()['tvdb']
        self.tmdb_key             = control.setting('tmdb_apikey')
        if self.tmdb_key == '' or self.tmdb_key == None: self.tmdb_key = base64.b64decode('YzdjZDA1NTJiY2VhMzRlMjQzY2E2YTlmNjIyMGI4Njc=')
        self.tmdb_lang            = 'en'
        self.datetime             = (datetime.datetime.utcnow() - datetime.timedelta(hours = 5))
    ...
    def tvshowDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                if not tvdb == "0" or tvdb == None: sysmetalliq = "plugin://plugin.video.metalliq/tv/add_to_library_parsed/%s/direct.keltec.q" % tvdb
                elif "tt" in imdb: sysmetalliq = "plugin://plugin.video.metalliq/tv/add_to_library_parsed/%s/direct.keltec.q" % imdb
                else: sysmetalliq = "0"

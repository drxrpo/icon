...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(plugin.video.underdog)'):
        return os.path.join(xbmcaddon.Addon('plugin.video.underdog').getAddonInfo('path'), 'resources', 'skins', 'theme', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(plugin.video.underdog)') else setting('appearance.alt')
    if appearance == "underdog": appearance = "Default"
    return appearance


def artwork():
    execute('RunPlugin(plugin://plugin.video.underdog)')

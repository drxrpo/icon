...
from underdogscrapers.common import clean_title
...
class HostedLink:
    def __init__(self, title, year, imdb=None, tvdb=None, host=None, include_disabled=False, timeout=30, exclude=None, enable_debrid=False):
        ...
        xbmcvfs.mkdir(xbmc.translatePath(xbmcaddon.Addon("script.module.underdogscrapers").getAddonInfo('profile')))
    ...
    def __get_scrapers(self, include_disabled, exclude):
        klasses = underdogscrapers.relevant_scrapers(self.host, include_disabled, exclude=exclude)
        scrapers = []
    ...
    def get_url(scraper, title, show_year, year, season, episode, imdb, tvdb, type, cache_location, maximum_age, check_url = False, debrid = False):
        cache_enabled = xbmcaddon.Addon('script.module.underdogscrapers').getSetting("cache_enabled") == 'true'
        try:
